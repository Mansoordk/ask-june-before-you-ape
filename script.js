  ...
  return `
    <div class="card">
      ...
      <div class="tokenomics-grid">
        ...
        <div class="token-box">
          <span>📉 From ATH</span>
          <strong style="color:${token.athChange >= 0 ? '#22c55e' : '#ef4444'}">${formatPercent(token.athChange)}</strong>
        </div>
        <div class="token-box">
          <span>📅 ATH Date</span>
          <strong>${token.athDate ? new Date(token.athDate).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }) : "N/A"}</strong>
        </div>
        <div class="token-box">
        ...
        <div class="token-box">
          <span>🚀 From ATL</span>
          <strong style="color:${token.atlChange >= 0 ? '#22c55e' : '#ef4444'}">${formatPercent(token.atlChange)}</strong>
        </div>
        <div class="token-box">
          <span>📅 ATL Date</span>
          <strong>${token.atlDate ? new Date(token.atlDate).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }) : "N/A"}</strong>
        </div>
      </div>
    </div>
  `;
  ...
  return `
    <div class="card">
      ...
      <div class="tokenomics-grid">
        <div class="token-box">
          <span>24 Hours</span>
          <strong style="color:${color(token.change24h)}">${format(token.change24h)}</strong>
        </div>
        <div class="token-box">
          <span>7 Days</span>
          <strong style="color:${color(token.change7d)}">${format(token.change7d)}</strong>
        </div>
        <div class="token-box">
          <span>30 Days</span>
          <strong style="color:${color(token.change30d)}">${format(token.change30d)}</strong>
        </div>
        <div class="token-box">
          <span>1 Year</span>
          <strong style="color:${color(token.change1y)}">${format(token.change1y)}</strong>
        </div>
      </div>
    </div>
  `;
...
function renderPortfolioChart(portfolio) {
  ...
  const colors = [
    "#f7931a", "#627eea", "#14f195", "#6fbcf0", "#8b5cf6",
    ...
    try {
      ...
      on("downloadBtn", "click", async () => {
        try {
          ...
          link.download = `${project}-analysis.png`;
